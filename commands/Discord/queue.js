				...
				if (currentIn >= guilds[message.channel.id].maxIn) {
					...
					message.reply(
						'**' +
							setinNum +
							' : ' +
							guilds[message.channel.id].queue[setinNum - 1] +
							' [' +
							guilds[message.channel.id].queueContent[setinNum - 1] +
							']** is now **IN**'
					);
					queue_list(message);
					return;
  ...
  `,
		...
		for (let i = 0; i < guilds[message.channel.id].queue.length; i++) {
			...
			console.log(
				j +
					' - ' +
					guilds[message.channel.id].queue[i] +
					'(' +
					guilds[message.channel.id].queueId[i] +
					')' +
					' : ' +
					guilds[message.channel.id].queueContent[i]
			);
		}
		queue_list(message);
	}
		...
		if (guilds[message.channel.id].isIn.length === 0 || guilds[message.channel.id].queueId.indexOf(member) === -1) {
			...
			for (let i = 1; i < guilds[message.channel.id].queue.length + 1; i++) {
				...
				else if (guilds[message.channel.id].queue[i - 1] === member_Id) {
					if (guilds[message.channel.id].isIn[i - 1] === isInNum) {
						message.reply(
							'*You are already IN* with' +
								'**  ' +
								i +
								' : ' +
								guilds[message.channel.id].queue[i - 1] +
	...
	async function queue_list(message) {
		...
		if (guilds[message.channel.id].queueContent.length === 0) {
			if (parseInt(guilds[message.channel.id].maxIn, 10) === 1) {
				...
				message.reply(
					`\`\`\`css\n[There are ${
						Global.numArray[guilds[message.channel.id].maxIn]
					} free spots]\nNo one is IN to the queue currently\`\`\``
				);
			}
		}
			...
			for (let i = 0; i < guilds[message.channel.id].queueContent.length; i++) {
				const temp =
					...
					guilds[message.channel.id].queueContent[i] +
					'] ' +
					(guilds[message.channel.id].isIn[i] === 1 ? '  ' + '<= current IN' : '') +
					'\n';
				if ((message2 + temp).length <= 2000 - 3) {
					message2 += temp;
				}
	...
	function queue_delete(member, deleteNum, message) {
		deleteNum = parseInt(deleteNum, 10);
		const queueLength = guilds[message.channel.id].queueContent.length;
		const deletedName = guilds[message.channel.id].queue;
		const deletedContent = guilds[message.channel.id].queueContent;
		if (deleteNum && deleteNum >= 1) {
			if (deleteNum > queueLength) {
				...
				message.reply(
					'Queue on ' +
						...
						deleteNum +
						' : ' +
						deletedName[deleteNum - 1] +
						(deletedContent[deleteNum - 1] ? '  [' + deletedContent[deleteNum - 1] + ']**' : '**') +
						' deleted!'
				);
			...
			return message.reply(
				'Please set appropriate delete number ' + (queueLength > 1 ? '[ 1 ~  ' + queueLength + ' ]' : ' [ 1 ]')
			);
		}
	}
	...
	function queue_insert(member, member_Id, message) {
		...
		if (guilds[message.channel.id].queue.length === 0) {
			...
			if (switchNum > guilds[message.channel.id].queueContent.length) {
				...
				message.reply(
					'Inserted to Queue number:  ' +
						'**' +
						switchNum +
						(guilds[message.channel.id].queueContent[switchNum - 1]
							? '  [' + guilds[message.channel.id].queueContent[switchNum - 1] + ']**'
							: '**')
				);
			}
