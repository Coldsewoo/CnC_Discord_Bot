...
`,
		...
		if (resourceAvailable === undefined || resourceAvailable.toLowerCase().match(/[mbt]/) === null || resourceAvailable.match(/\D/g).length > 1) return message.reply('wrong format!');
		...
		milestone -= 1;
		stoneReqtoMilestone = meddata[milestone * 10].StoneSum;
		const remains = numeral((resourceAvailable - stoneReqtoMilestone + currentLvRes) * numBuildings).format('0.0a').toUpperCase();
