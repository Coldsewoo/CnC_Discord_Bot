...
Always Online   -  ${guildsheet[5][16]}\`\`\``,
			...
			if (['level', 'lv', 'guild'].indexOf(content[0]) >= 0) {
				message.channel.send({
					embed: {
						color: `${color}`,
						author: {
...
Always Online   -  ${guildsheet[5][17]}\`\`\``,
					...
					if (['wood', 'sawmill', 'woodcutting'].indexOf(content[0]) >= 0) {
						message.channel.send({
							embed: {
								color: `${color}`,
								author: {
...
Always Online   -  ${guildsheet[5][19]}\`\`\``,
						...
						if (['stone', 'warehouse', 'wh'].indexOf(content[0]) >= 0) {
							message.channel.send({
								embed: {
									color: `${color}`,
									author: {
...
Always Online   -  ${guildsheet[5][21]}\`\`\``,
							...
							if (['points', 'fortress', 'asc', 'ascension', 'point'].indexOf(content[0]) >= 0) {
								message.channel.send({
									embed: {
										color: `${color}`,
										author: {
...
Always Online   -  ${guildsheet[5][27]}\`\`\``,
								...
								if (['tower', 'sacrifical', 'offerings', 'offering', 'so'].indexOf(content[0]) >= 0) {
									message.channel.send({
										embed: {
											color: `${color}`,
											author: {
...
Always Online   -  ${guildsheet[5][28]}\`\`\``,
												...
												if (['space', 'academy', 'ship', 'sa'].indexOf(content[0]) >= 0) {
													message.channel.send({
														embed: {
															color: `${color}`,
															author: {
...
Always Online   -  ${guildsheet[5][29]}\`\`\``,
													...
													if (content[0] === 'highest') {
														var petHigh = guildsheet[7][2].split(",");
														var ascHigh = guildsheet[7][3].split(",");
														var shipHigh = guildsheet[7][4].split(",");
														var woodHigh = guildsheet[7][5].split(",");
														var stoneHigh = guildsheet[7][6].split(",");
														var soHigh = guildsheet[7][7].split(",");
														message.channel.send({
															embed: {
																color: `${color}`,
																author: {
																...
																title: '**Highest Bonus**',
																fields: [{
																	name: 'Shows guild that has highest bonus amount',
																	value: `\`\`\`prolog
Woodcutting - ${woodHigh[1] + " " + numeral(woodHigh[0]).format('0.00%')}
Stone Yield - ${stoneHigh[1] + " " + numeral(stoneHigh[0]).format('0.00%')}
Sac EXP     - ${soHigh[1] + " " + numeral(soHigh[0]).format('0.00%')}
Ship Arena  - ${shipHigh[1] + " " + numeral(shipHigh[0]).format('0.00%')}
Asc Points  - ${ascHigh[1] + " " + ascHigh[0]}
Pet Arena   - ${petHigh[1] + " " + numeral(petHigh[0]).format('0.00%')}\`\`\``,
																}
																],
